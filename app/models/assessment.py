...
class ExamSession(db.Model):
    ...
    skill_id = db.Column(db.Integer, db.ForeignKey('skill_categories.id'), nullable=True) # Allowed to be null temporarily for migration
    status = db.Column(db.String(20), nullable=False, default='draft') # 'draft', 'submitted', 'graded'
    started_at = db.Column(db.DateTime, default=datetime.utcnow)
    submitted_at = db.Column(db.DateTime, nullable=True)
    total_score = db.Column(db.Integer, nullable=True)

    skill = db.relationship('SkillCategory', backref=db.backref('exam_sessions', lazy=True))
